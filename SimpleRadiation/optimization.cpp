...
double evaluateOptimizationFunctionBandRandF(const double* vector, double* nu, double* observedInu, double* Ee, double**** dFe, int Np, int Nnu, int Nd, double*** Bn, double*** sintheta, int*** thetaIndex, double*** concentrations, double**** Inu, double**** Anu, double*** area, double*** length, const double& epsilonB){
	double r = vector[1]*maxR;
	double B = vector[0]*maxB;
...
void findMinParametersBandRandF(double* vector, const double* grad, double* nu, double* observedInu, double* Ee, double**** dFe, int Np, int Nnu, int Nd, double*** Bn, double*** sintheta, int*** thetaIndex, double*** concentrations, double**** Inu, double**** Anu, double*** area, double*** length, double& currentF, const double& epsilonB){
	const int Ngrad = 3;
	double minVector[Ngrad];
	double tempVector1[Ngrad];
	double tempVector2[Ngrad];
	...
	double f = evaluateOptimizationFunctionBandRandF(tempVector1, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
	if(f > currentF){
		int count = 0;
		while (f > currentF && count < 20){
			count++;
			...
			f = evaluateOptimizationFunctionBandRandF(tempVector1, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
		}
	...
	f = evaluateOptimizationFunctionBandRandF(vector, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
	...
	double f3 = evaluateOptimizationFunctionBandRandF(tempVector1, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
	...
	for(int j = 0; j < 10; ++j) {
		...
		double f1 = evaluateOptimizationFunctionBandRandF(tempVector1, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
		double f2 = evaluateOptimizationFunctionBandRandF(tempVector2, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
	...
	double tempF = evaluateOptimizationFunctionBandRandF(tempVector1, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
...
void optimizeParametersBandRandF(double* vector,  double* nu, double* observedInu, double* Ee, double**** dFe, int Np, int Nnu, int Nd, double*** Bn, double*** sintheta, int*** thetaIndex, double*** concentrations, double**** Inu, double**** Anu, double*** area, double*** length, const double& epsilonB, FILE* logFile){
	const int Ngrad = 3;
	double minVector[Ngrad];
	double tempVector[Ngrad];
	minVector[0] = minB/maxB;
	minVector[1] = minR/maxR;
	minVector[2] = minFraction;
	double currentF = evaluateOptimizationFunctionBandRandF(vector, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
	printf("optimization function = %g\n", currentF);
	fprintf(logFile, "optimization function = %g\n", currentF);
	printf("Bfactor = %g rmax = %g\n", vector[0]*maxB, vector[1]*maxR);
	fprintf(logFile, "Bfactor = %grmax = %g\n", vector[0]*maxB, vector[1]*maxR);
	for(int k = 0; k < Niterations; ++k) {
		...
		/*for(int j = 0; j < 5; ++j){
			...
			double tempF = evaluateOptimizationFunctionBandRandF(tempVector, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
			if(tempF < currentF){
				currentF = tempF;
				for(int i = 0; i < Ngrad; ++i) {
					vector[i] = tempVector[i];
		...
		for(int i = 0; i < Ngrad; ++i) {
			...
			tempVector[i] = vector[i] + dx;
			//currentF = evaluateOptimizationFunction5(vector[0]*maxB, vector[1]*maxN, vector[2]*maxFraction, vector[3]*maxR, vector[4]*maxV, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Nmonth, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length);
			double f = evaluateOptimizationFunctionBandRandF(tempVector, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
			tempVector[i] = vector[i] - dx;
			double f1 = evaluateOptimizationFunctionBandRandF(tempVector, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
		...
		findMinParametersBandRandF(vector, grad, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, currentF, epsilonB);

		currentF = evaluateOptimizationFunctionBandRandF(vector, nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, epsilonB);
		//valley second step
		...
		printf("optimization function = %g\n", currentF);
		fprintf(logFile, "optimization function = %g\n", currentF);
		printf("Bfactor = %g rmax = %g fraction = %g\n", vector[0]*maxB, vector[1]*maxR, vector[2]);
		fprintf(logFile, "Bfactor = %g rmax = %g fraction = %g\n", vector[0]*maxB, vector[1]*maxR, vector[2]);
	}
	printf("finish optimization\n");
	fprintf(logFile, "finish optimization\n");
}
...
void optimizeParameterB(double& B, double* nu, double* observedInu, double* Ee, double**** dFe, int Np, int Nnu, int Nd, double*** Bn, double*** sintheta, int*** thetaIndex, double*** concentrations, double**** Inu, double**** Anu, double*** area, double*** length, const double& R, const double& fraction, const double& epsilonB, FILE* logFile){
	double error = evaluateOptimizationFunction(nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, B, R, fraction, epsilonB);
	printf("optimization function = %g\n", error);
	fprintf(logFile, "optimization function = %g\n", error);
	const double Nb = 100;
	...
	B = (B1 + B2)/2;
	error = evaluateOptimizationFunction(nu, observedInu, Ee, dFe, Np, Nnu, Nd, Bn, sintheta, thetaIndex, concentrations, Inu, Anu, area, length, B, R, fraction, epsilonB);
	printf("optimization function = %g\n", error);
	fprintf(logFile, "optimization function = %g\n", error);
	printf("Bfactor = %g\n", B);
