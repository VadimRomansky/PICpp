...
#pragma omp parallel for
	...
	double a = scalarMultiplyLargeVectors(resultBasis[0], tempVector, xnumber, lnumber);
	...
	if (outHessenbergMatrix[n - 1][n - 2] > 0) {
		...
		a = scalarMultiplyLargeVectors(resultBasis[0], tempVector, xnumber, lnumber);
		a = scalarMultiplyLargeVectors(tempVector, tempVector, xnumber, lnumber);
	} else {
