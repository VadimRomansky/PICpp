...
concentrations = importdata('concentrationsYZ_5.dat');
...
for i=1:Ny,  
   ...
   for j = 1:Nz,
       charge_density(i, j) = concentrationsYZ(Nz*(i-1) + j + c*N, 1);
       charge_density_hat(i, j) = concentrationsYZ(Nz*(i-1) + j + c*N, 2);
       for t = 1:Ntypes,
           particle_concentrations(i, j, t) = concentrationsYZ(Nz*(i-1) + j + c*N, 2 + t);
       end;
   end;
end;
