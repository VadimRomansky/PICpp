...
concentrations = importdata('concentrationsXY_5.dat');
...
for i=1:Nx,  
   ...
   for j = 1:Ny,
       charge_density(i, j) = concentrationsXY(Ny*(i-1) + (j-1) + 1 + c*N, 1);
       charge_density_hat(i, j) = concentrationsXY(Ny*(i-1) + (j-1) + 1 + c*N, 2);
       for t = 1:Ntypes,
           particle_concentrations(i, j, t) = concentrationsXY(Ny*(i-1) + (j-1) + 1 + c*N, 2 + t);
       end;
   end;
end;
