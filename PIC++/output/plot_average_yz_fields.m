...
for i=1:Nx,
   ...
   Ez(i,3) = 0;
    for j=1:Ny,
        for k=1:Nz,
            Ex(i,1) = Ex(i,1) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + a*NE, 1);
            Ex(i,2) = Ex(i,2) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + b*NE, 1);
            Ex(i,3) = Ex(i,3) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + c*NE, 1);
   
            Ey(i,1) = Ey(i,1) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + a*NE, 2);
            Ey(i,2) = Ey(i,2) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + b*NE, 2);
            Ey(i,3) = Ey(i,3) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + c*NE, 2);
   
            Ez(i,1) = Ez(i,1) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + a*NE, 3);
            Ez(i,2) = Ez(i,2) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + b*NE, 3);
            Ez(i,3) = Ez(i,3) + Efield((Nz)*(Ny)*(i-1) + (Nz)*(j-1) + k + c*NE, 3);
        end;
    end;
            Ex(i,1) = Ex(i,1)/(Ny*Nz);
            Ex(i,2) = Ex(i,2)/(Ny*Nz);
...
for i = 1:Nx-1,
   ...
   for j = 1:Ny-1,
       for k = 1:Nz-1,
            Bx(i, 1) = Bx(i, 1) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + a*NB, 1);
            Bx(i, 2) = Bx(i, 2) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + b*NB, 1);
            Bx(i, 3) = Bx(i, 3) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + c*NB, 1);
            By(i, 1) = By(i, 1) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + a*NB, 2);
            By(i, 2) = By(i, 2) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + b*NB, 2);
            By(i, 3) = By(i, 3) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + c*NB, 2);
            Bz(i, 1) = Bz(i, 1) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + a*NB, 3);
            Bz(i, 2) = Bz(i, 2) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + b*NB, 3);
            Bz(i, 3) = Bz(i, 3) + Bfield(((Nz-1)*(Ny-1)*(i-1) + (Nz-1)*(j-1) + k) + c*NB, 3); 
       end;
   end;
   Bx(i, 1) = Bx(i, 1)/((Ny-1)*(Nz-1));
   Bx(i, 2) = Bx(i, 2)/((Ny-1)*(Nz-1));
   Bx(i, 3) = Bx(i, 3)/((Ny-1)*(Nz-1));
   By(i, 1) = By(i, 1)/((Ny-1)*(Nz-1));
   By(i, 2) = By(i, 2)/((Ny-1)*(Nz-1));
   By(i, 3) = By(i, 3)/((Ny-1)*(Nz-1));
   Bz(i, 1) = Bz(i, 1)/((Ny-1)*(Nz-1));
   Bz(i, 2) = Bz(i, 2)/((Ny-1)*(Nz-1));
   Bz(i, 3) = Bz(i, 3)/((Ny-1)*(Nz-1));
