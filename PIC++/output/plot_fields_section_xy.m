clear;
EfieldXY = importdata('EfieldXY.dat');
BfieldXY = importdata('BfieldXY.dat');
load Xfile.dat;
load Yfile.dat;
load Zfile.dat;
...
for i=1:Nx-1,
   ...
   Xgrid(i) = (Xfile(i) - Xfile(2));
    for j = 1:Ny-1,
        Ex(i,j) = EfieldXY((Ny-1)*(i-1) + j + c*NE, 1);
        Ey(i,j) = EfieldXY((Ny-1)*(i-1) + j + c*NE, 2);
        Ez(i,j) = EfieldXY((Ny-1)*(i-1) + j + c*NE, 3);
    end;
end;
